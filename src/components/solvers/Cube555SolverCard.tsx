...
export function Cube555SolverCard({ initialScramble }: { initialScramble?: string } = {}) {
  ...
  useEffect(() => {
    setScramble(s => s || generateScramble('555'))
  }, [])
  ...
  function generate() {
    ...
    setScramble(generateScramble('555'))
    setGenerating(false)
  }
  ...
  async function handleSolve() {
    ...
    setAnim(null)
    try {
      const res = await fetch(`${settings.ml_service_url}/solve/555`, {
        method: 'POST',
  ...
  function animateStage(res: Result555, stageIdx: number) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs mb-3" style={{ color: 'var(--text-secondary)' }}>
            Reduction method: solve the 6 centers (both orbits), attach the 24 wings to
            their central edges, then finish as a 3×3 with the CFOP solver.
          </p>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {solving ? 'Solving… (minutes; rare cases longer)' : 'Solve'}
            </button>
          </div>
        </div>
      </div>
      ...
      {result && !solving && (
        <div className="mt-4 pt-4 border-t flex flex-col gap-2" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
            <span>reduction {result.reduction_moves}m · then 3×3 CFOP</span>
